...
// ── VERİ KAYIT ─────────────────────────────────────────────────────────────
// saveData encryption bloğunda tanımlı (AES-256-GCM)
...
// Anahtar IndexedDB'de extractable:false olarak saklanır — localStorage'a asla yazılmaz
// Şifrelenmiş veri hâlâ localStorage'da durur ama anahtarsız işe yaramaz
...
async function getCryptoKey(){
  ...
  // Eski localStorage anahtarını temizle (tek seferlik migrasyon)
  localStorage.removeItem(CRYPTO_KEY_NAME);
  try{
    // IndexedDB'den non-extractable key'i yükle
    const stored=await _idbGet(CRYPTO_KEY_NAME);
    if(stored){
      _cryptoKey=stored; // zaten CryptoKey nesnesi, export edilemez
    } else {
      // İlk kez: üret ve IndexedDB'ye göm — raw bytes asla dışarı çıkmaz
    ...
    // IndexedDB erişilemiyorsa (private mode vb.) session-only key üret
    if(!_cryptoKey)
      _cryptoKey=await crypto.subtle.generateKey({name:'AES-GCM',length:256},false,['encrypt','decrypt']);
  }
  return _cryptoKey;
...
async function encryptData(obj){
  try{
    ...
    // Şifreleme başarısız — plaintext yazmak yerine hatayı yukarı fırlat
...
async function decryptData(str){
  try{
    ...
    // Şifreli değilse düz JSON olarak oku (geçiş dönemi)
...
// saveData — şifreleme başarısız olursa veri hiç yazılmaz, kullanıcı bilgilendirilir
...
// PIN'i SHA-256 ile hash'leyip sakla — plaintext asla localStorage'a yazılmaz
...
async function verifyPin(input,storedHash){
  // Migrasyon: eski plaintext PIN (4 karakter) → hash'e otomatik yükselt
  // SHA-256 base64 her zaman 44 karakterdir; 4 karakter ise eski plaintext demektir
  if(storedHash&&storedHash.length===4){
    if(input!==storedHash)return false;
    // Eşleşti — sessizce hash'e çevir, kullanıcı fark etmez
...
// _pinDisableBuf modül kapsamında — window'a asla yazılmaz
let _pinDisableBuf='';
